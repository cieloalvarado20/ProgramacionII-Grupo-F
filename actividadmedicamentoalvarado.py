...
def guardar_en_archivo():
    with open("medicamento.txt","w",encoding="utf-8") as archivo:
        for Medicamento in Medicamentos_data:
            archivo.write(f"{Medicamento['Nombre']}|{Medicamento['Presentacion']}|{Medicamento['Dosis']}|"
...
def cargar_desde_archivo_medicamento():
    try:
        ...
        open("medicamentos.text", "w", encoding="utf-8").close()
def eliminar_medicamentos():
    ...
    if seleccionado:
        ...
        if messagebox.askyesno("Eliminar Medicamento",f"¿ Esta seguro de eliminar el Medicamento'{treeview.item(id_item,'values')[0]}'?"):
            ...
            cargar_treeview()
            messagebox.showinfo("Eliminar","medicamento eliminado exitosamente.")
        else: #este else es del if selecionado
            messagebox.showwarning("Eliminar", "No se a seleccionado ningun medicamento.")
            return
...
btn_eliminar = ttk.Button(btn_frame, text="Eliminar",command=eliminar_medicamentos)
btn_eliminar.grid(row=0, column=1, padx=5, pady=5, sticky="ew")

treeview=ttk.Treeview(form_frame, columns=("Nombre", "Presentacion","Dosis", "Fecha vencimiento"), show="headings")
# Frame lista
